...
namespace UnityEngine.Rendering.Universal
{
    ...
    /// XRDisplaySubsystem URP itself builds its XR passes from, so this is independent of
    /// which XR plugin provides it (OpenXR and the Oculus plugin both implement it).
    /// </remarks>
    internal static class VideoLabUrpHook
    {
        ...
        public static void CaptureFrame(CommandBuffer cmd, UniversalCameraData cameraData)
        {
        ...
        static RenderTexture SwapchainTexture()
        {
            ...
            // the same display URP builds its XR passes from (XRSystemUniversal), so this
            // does not care which plugin provides it
            XR.XRDisplaySubsystem display = XRSystem.GetActiveDisplay();
            if (display == null || display.GetRenderPassCount() < 1)
                return null;
